...
async function getAdminDashboardMetrics() {
  ...
  const listingsPromise = db.collection('produceListings').where('status', '==', 'available').get();
  const escrowPromise = db.collection('groupBuys').where('status', '==', 'pending_delivery').get();

  const sevenDaysAgo = subDays(new Date(), 7);
  const recentGroupsPromise = db
    .collection('groupBuys')
    .where('createdAt', '>=', sevenDaysAgo)
    .get();

  const [usersSnapshot, groupsSnapshot, listingsSnapshot, escrowSnapshot, recentGroupsSnapshot] =
    await Promise.all([
      usersPromise,
      groupsPromise,
      listingsPromise,
  ...
  const userRoles = usersSnapshot.docs.map((doc) => doc.data().role);
  const buyerCount = userRoles.filter((role) => role === 'buyer').length;
  const farmerCount = userRoles.filter((role) => role === 'farmer').length;

  const recentGroups = recentGroupsSnapshot.docs.map((doc) => {
    const data = doc.data();
    return {
      ...data,
  ...
  return (
    <div>
      ...
      <div className='grid gap-6 md:grid-cols-2 lg:grid-cols-4'>
        ...
        <Card>
          <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-2'>
            <CardTitle className='text-sm font-medium'>Completed Groups</CardTitle>
      ...
      <div className='mt-8'>
        <AdminChart recentGroupData={metrics.recentGroupData} userRoleData={metrics.userRoleData} />
      </div>
    </div>
  );
}
